...
function NavBarPresentationMenu({ menuItemData, sitename }) {
    ...
    items.map((menuItem) => (
      <MenuItem key={`HeaderMenu_${menuItem.title}`}>
        <Link
          ...
          className="data-focus:bg-charcoal-100 text-charcoal-900 flex w-full rounded-lg"
        >
  ...
  return hasItems ? (
    <Menu data-testid="NavBar-Menu" as="div" className="relative inline-block">
      <MenuButton data-testid={`${transKey}-button`} className={buttonStyling}>
        {generateButtonContents()}
      </MenuButton>

      <MenuItems className="absolute top-10 w-auto min-w-full right-0 p-2 transition duration-100 ease-out transform lg:-translate-x-0 bg-white rounded-lg shadow-lg ring-1 ring-black/50 focus:outline-hidden">
