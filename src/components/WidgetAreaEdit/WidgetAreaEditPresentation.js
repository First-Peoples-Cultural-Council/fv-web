...
function WidgetAreaEditPresentation({
  ...
  return (
    <div data-testid="WidgetAreaEdit">
      {widgetIds?.length > 0 ? (
        <main className="flex-1">
            ...
            <div className="flex justify-between items-center p-4">
              <div>
                <p className="text-fv-charcoal-light">
                  The Widgets that appear on your <span className="italic font-bold">{destination?.title}</span> page
                </p>
              </div>
              <button
          ...
          <div className="grid grid-cols-8">
            ...
            >
              <SortableContainer.Presentation items={widgetIds} setItems={setWidgetIds}>
                {widgetIds?.map((id, index) => (
                  <SortableItem.Presentation key={`sortable-${id}-${index}`} id={id} handle>
                    <div className="flex w-full items-center">
                      <div
                        onClick={() => setCurrentWidget(widgetData?.[id])}
                        className={`${
                          currentWidget?.uid === widgetData?.[id]?.uid ? 'border-4 border-primary' : 'hover:bg-gray-50'
                      ...
                      >
                        <div className="grid grid-cols-6 gap-6 text-left">
                          <div className="flex items-center text-left col-span-1">
                            {useWidgetIcon(widgetData?.[id]?.type, 'w-12 h-12 fill-current text-primary')}
                          </div>
                          <div className="flex items-center text-left col-span-3">
                            <div className="truncate">
                              ...
                              <p className="text-fv-charcoal-light">{widgetData?.[id]?.title}</p>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center text-left">
                          {getIcon(widgetData?.[id]?.visibility, 'w-5 h-5 fill-current text-primary')}
                        </div>
                      </div>
                      <div
                        className={`inline-flex ${
                          currentWidget?.uid === widgetData?.[id]?.uid ? 'opacity-100' : 'opacity-0'
                        }`}
                      >
                        {getIcon('ChevronRight', 'fill-current h-10 w-10 text-primary')}
                      </div>
                    </div>
        ...
        <main data-testid="WidgetAreaEdit" className="max-w-7xl mx-auto flex w-full h-screen">
          <div className="w-full space-y-4 mt-16">
            <div className="text-center">
              Your page currently has no widgets on it. Click the button below to add one.
            </div>
            <button
              onClick={() => setAddModalOpen(true)}
      ...
      {/* Add Modal */}
      <Modal.Presentation isOpen={addModalOpen} closeHandler={() => setAddModalOpen(false)}>
