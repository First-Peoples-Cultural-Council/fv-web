...
function SortByHeader({ title, asc, desc }) {
  ...
  return (
    <button
      ...
      onClick={() => handleClick()}
      className={`${isActive ? 'font-bold text-blumine-500' : 'text-charcoal-500'} group inline-flex items-center cursor-pointer`}
    >
      <span className="text-left capitalize">{title}</span>
      {getIcon(
        iconName,
        'fill-current ml-1 h-5 w-5 flex-none rounded-sm fill-current group-hover:text-charcoal-900 group-focus:text-charcoal-900',
      )}
    </button>
  )
}
