...
function DashboardImmersionPresentation({ headerContent, isLoading, tileContent, labels, site }) {
          ...
          <div className="col-span-3">
            <DashboardTable.Presentation
              ...
              tableBody={labels.map((label) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap text-fv-charcoal">{label?.immersionLabel || '-'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-fv-charcoal">{label?.english}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-fv-charcoal">
                    <div className="flex justify-center">{getIndicatorIcon(label?.relatedAudio)}</div>
                  </td>
