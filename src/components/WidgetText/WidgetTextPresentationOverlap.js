...
function WidgetTextPresentationOverlap({ widgetData }) {
  const { title, textWithFormatting, image, url, urlLabel, audio } = widgetData?.settings
  const light = false
  ...
  return (
    <div className={`relative py-6 bg-${bgColor2}`}>
      <div className={`hidden absolute top-0 inset-x-0 h-1/2 bg-${bgColor} lg:block`} aria-hidden="true" />
      <div className="max-w-7xl mx-auto bg-secondary lg:bg-transparent lg:px-8">
        <div className="lg:grid lg:grid-cols-12">
          <div className="relative z-10 lg:col-start-1 lg:row-start-1 lg:col-span-5 lg:py-16 lg:bg-transparent">
            <div className={`absolute inset-x-0 h-1/2 bg-${bgColor}  lg:hidden`} aria-hidden="true" />
          ...
          >
            <div className="relative max-w-md mx-auto py-12 px-4 space-y-6 sm:max-w-3xl sm:py-16 sm:px-6 lg:max-w-none lg:p-0 lg:col-start-4 lg:col-span-6">
              <h2 className={`text-3xl font-extrabold text-${accentColor}`} id="join-heading">
                {title}
                {audio && (
                  <AudioButton
                    audioArray={[audio]}
