...
function DashboardPagesPresentation({ headerContent, isLoading, tileContent, customPages, site, sitename }) {
      ...
      <DashboardLanding.Presentation tileContent={tileContent} headerContent={headerContent} site={site}>
        <DashboardTable.Presentation
          ...
          tableBody={customPages.map((page) => (
            <tr key={page.id}>
              <td className="px-6 py-4 whitespace-normal text-sm font-medium text-gray-900">{page.title}</td>
              <td className="px-6 py-4 whitespace-normal text-sm text-fv-charcoal">{page.subtitle}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-fv-charcoal">{page.url}</td>
