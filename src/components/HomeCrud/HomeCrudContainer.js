...
function HomeCrudContainer() {
  const { backHandler, dataToEdit, isWidgetAreaEdit, site, submitHandler } = HomeCrudData()
  return isWidgetAreaEdit ? (
    <Loading.Container isLoading={site?.uid ? false : true}>
      <HomeCrudPresentation site={site} />
    </Loading.Container>
  ) : (
    <HomeForm cancelHandler={backHandler} submitHandler={submitHandler} dataToEdit={dataToEdit} />
  )
}
