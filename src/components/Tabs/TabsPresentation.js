...
function TabsPresentation({ accentColor, selectedValue, tabs, setValue }) {
  ...
  return (
    <>
      <nav className="hidden sm:flex items-center relative w-auto rounded-lg" aria-label="Tabs">
        {tabs?.map((tab, tabIndex) => (
          ...
          >
            {tab?.icon !== 'All' ? getIcon(tab?.icon, 'inline-flex fill-current w-5 h-5 mr-2') : ''}
            {tab?.transKey ? t(tab?.transKey) : tab?.label}
          </button>
        ))}
      </nav>
      <div className="sm:hidden flex-1 w-52 ">
        <Listbox.Presentation selectedValue={selectedValue} options={tabs} setValue={setValue} />
      </div>
    </>
  )
}
