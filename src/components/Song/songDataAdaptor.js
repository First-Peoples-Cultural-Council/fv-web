function songDataAdaptor(data) {
  ...
  const songContextParams = data?.contextParameters?.book
  ...
  if (songContextParams?.authors?.length > 1) {
    for (let i = 1; i < songContextParams?.authors?.length; i++) {
      author += `, ${songContextParams?.authors?.[i]?.['dc:title']}`
    }
  }
  ...
  const pages = songContextParams?.pages
  const pagesHaveContent =
    pages?.filter(
      (page) => page?.content !== null || page?.contentTranslation?.length > 0,
    )?.length > 0
  ...
  const entry = {
    ...
    titleTranslation:
      properties['fvbook:title_literal_translation']?.[0]?.translation || '',
    audio: properties['fv:related_audio'] || [],
    coverVisual,
    introduction: properties['fvbook:introduction'] || '',
    introductionTranslation:
      properties['fvbook:introduction_literal_translation']?.[0]?.translation ||
      '',
