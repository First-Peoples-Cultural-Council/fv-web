...
function SliderPresentation({ children, handlePrev, handleNext, slideProps, containerRef, hasNext, hasPrev }) {
  return (
    <div id="SliderWrapper" className="relative py-2 lg:py-6 overflow-x-auto lg:overflow-hidden">
...
const SlideButton = ({ onClick, type }) => (
  ...
  >
    <span className="w-10 text-white block mx-auto">
      {getIcon(`${type === 'prev' ? 'ChevronLeft' : 'ChevronRight'}`, 'fill-current h-full w-auto')}
    </span>
  </button>
)
