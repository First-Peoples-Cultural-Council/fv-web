...
function WidgetQuotesPresentation({ widgetData }) {
  const { backgroundImage, quote1, quote1By, quote2, quote2By, quote3, quote3By } = widgetData?.settings
  ...
  const makeQuote = (qoute, quoteBy) => {
    return (
      <div className="relative w-4/5 md:w-[30%] h-80 mt-24 md:my-28 bg-white rounded-t-xl rounded-br-xl flex flex-col justify-center items-center">
        ...
        <p className="text-primary font-bold text-center leading-5 w-1/2 pb-20 pt-4">{quoteBy}</p>
  ...
  return (
    <section
      ...
      style={{
        backgroundImage: `url(${getMediaUrl({ id: backgroundImage, type: 'gifOrImg' })}`,
