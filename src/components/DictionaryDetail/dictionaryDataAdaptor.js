import { getFriendlyDocType, extractTextFromHtml, convertStateToVisibility } from 'common/stringHelpers'
...
function dictionaryDataAdaptor(data) {
  ...
  const contextParams = data?.contextParameters?.word || data?.contextParameters?.phrase || {}
  ...
  if (hasBeenEditedInv2) {
    return {
      ...
      categories: contextParams?.categories || contextParams?.phrase_books || [],
    ...
    // V1_FUDGE
    const reference = properties['fv:reference'] ? `Reference: ${properties['fv:reference']}` : null
    const acknowledgement = properties?.['fv-word:acknowledgement'] || properties?.['fv-phrase:acknowledgement'] || null
    const generalNote = properties['fv:general_note'] ? extractTextFromHtml(properties['fv:general_note']) : null
    const literalTranslationsArray = properties?.['fv:literal_translation']?.map((trans) => trans.translation)
    const literalTranslations =
      literalTranslationsArray?.length > 0 ? `Literal Translations: ${literalTranslationsArray.join(', ')}` : null
    const sourcesArray = contextParams?.sources?.map((source) => source?.['dc:title'])
    const sources = sourcesArray?.length > 0 ? `Source: ${sourcesArray?.join(', ')}` : null
    ...
    return {
      ...
      relatedAssets:
        [].concat(contextParams?.related_phrases, contextParams?.related_assets).filter((item) => item != null) || [],
      audio: contextParams?.related_audio || [],
      images: contextParams?.related_pictures || [],
      videos: contextParams?.related_videos || [],
      partOfSpeech: contextParams?.part_of_speech || '',
