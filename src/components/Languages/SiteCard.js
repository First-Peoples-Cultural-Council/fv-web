...
function SiteCard({ site }) {
  ...
  let isLocked = site.visibility === MEMBERS

  let privateSiteModalOpenHandler = () => {
  ...
  return (
    <>
      ...
      ${isLocked ? 'hover:bg-phrase hover:border-phrase' : 'hover:bg-word hover:border-word'}`}
        to={`/${site?.sitename}`}
      ...
      >
        ...
        ></img>
        <p className="w-full ml-24 mr-8 md:mr-4 tracking-tight text-center break-word">{site.title}</p>
        {isLocked && getIcon('Lock', 'absolute bottom-2 right-2 h-5 w-5 fill-phrase-light group-hover:fill-white')}
      </Link>
      ...
      <Modal.Presentation isOpen={privateSiteModalOpen} closeHandler={() => setPrivateSiteModalOpen(false)}>
        <div
        ...
        >
          <div className="text-l font-medium text-fv-charcoal">
            <p className="text-xl">
              Private sites are currently unavailable on the new FirstVoices. Please try again later!
            </p>
            <p className="pt-2">
              To view this site now, you can find it by visiting{' '}
              <a href="https://www.firstvoices.com/explore/FV/sections/Data/" className="underline">
                the old FirstVoices Explore Languages page
              </a>{' '}
              and clicking on the site name. You will need to be logged in as a site member to see its private content.
              If you are not a member, you can request to join on that page.
            </p>
          </div>
          <button
            type="button"
